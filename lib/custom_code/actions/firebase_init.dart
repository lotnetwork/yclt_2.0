...
Future<void> _showNotification(RemoteMessage message) async {
  ...
  await flutterLocalNotificationsPlugin.show(1, message.notification!.title,
      message.notification!.body!.replaceAll(regExp, ''), notificationDetails,
      payload: payload);
...
void _configureDidReceiveLocalNotificationSubject(RemoteMessage message) {
  didReceiveLocalNotificationStream.stream
    ...
    if (action == "FLUTTER_NOTIFICATION_CLICK") {
      debugPrint(
          "Notification clicked! by person FLUTTER_NOTIFICATION_CLICK 1");
      // try {
      ...
      debugPrint(
          "Notification clicked! by person FLUTTER_NOTIFICATION_CLICK_STORY 1");
      // try {
      //   await NotificationRecord.collection
      //       .doc()
      //       .update(createNotificationRecordData(
...
Future<void> _configureSelectNotificationSubject(RemoteMessage message) async {
  selectNotificationStream.stream.listen((receive) async {
    ...
    if (action == "FLUTTER_NOTIFICATION_CLICK") {
      debugPrint(
          "Notification clicked! by person FLUTTER_NOTIFICATION_CLICK 2");
      //   try {
      //     await NotificationRecord.collection
      //         .doc()
      //         .update(createNotificationRecordData(
      ...
      debugPrint(
          "Notification clicked! by person FLUTTER_NOTIFICATION_CLICK_STORY 2");
